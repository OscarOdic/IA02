...
% Initialise le plateau avec le nombre de joueurs, l'orientation du plateau, et l'ajout des pièces
...
% L'ajout d'un pion dans une case libre sur une liste, suivant un indice, correspondant à la colonne du plateau
ajouter_colonne_init([(N,n)|Q], [(N,X)|Q], X, 1):-!.
ajouter_colonne_init([T|Q], [T|R], X, C):-Tmp is C-1, ajouter_colonne_init(Q, R, X, Tmp).

% L'ajout d'un pion dans une case, libre ou non, sur le plateau, suivant des coordonnées
ajouter_pion([T|Q], [Res|Q], X, (1,C)):-ajouter_colonne(T,Res,X,C),!.
ajouter_pion([T|Q], [T|R], X, (L,C)):-Tmp is L-1, ajouter_pion(Q, R, X, (Tmp,C)).

% L'ajout d'un pion dans une case, libre ou non, sur une liste, suivant un indice, correspondant à la colonne du plateau
...
% Récupère l'element d'une liste suivant son indice correspondant à la colonne d'un plateau
...
% Si le mouvement est possible, les coordonnées donnés pointe vers une case qui, soit est libre, soit on y trouve une pièce adverse
...
% Récupère les cases des pièces du joueur encore en jeu ainsi que les types des cases
casePionsLine([], _, [], _).
...
% Filtre une liste généré par le predicat precedent afin de ne garder que les coordonnées (en fonction du Khan ou non)
triPions([], []).
...
% Récupère tous les mouvements possibles d'un joueur sans prendre en compte le khan
possibleMoves(Board, Joueur, Mouvements, n):-
	pions(Board, Joueur, Pions),
	possibleMovePions(Board, Pions, Mouvements, Joueur),!.

% Récupère tous les mouvements possibles d'un joueur en prennant en compte le khan
...
% Permet à l'utilisateur de choisir une valeur entre Min et Max, se repete tant qu'il choisit une valeur incorrecte
choix(S, N, Min, Max):-
	repeat,
	write(S),
	read(Tmp),
...
% Le tour d'un joueur humain, pour le déplacement d'une pièce, sans prendre en compte le Khan
...
% Le tour d'un joueur humain, ou aucun mouvement n'est possible à cause du Khan, et où aucun pions n'a été perdu
...
% Le tour d'un joueur humain, ou aucun mouvement n'est possible à cause du Khan, mais où au moins un pion a été perdu
tourJoueur(Board, Res, Joueur, Khan, NewKhan):-
	possibleMoves(Board, Joueur, [], Khan),
	repeat,
...
% Recupere a partir d'une liste de mouvements uniquement les positions possibles d'arrivés (sans les positions de départ des pièces)
listeMouvements([], []):-!.
...
% Récupere les mouvement possible de la kalista à partir d'une liste de mouvements
...
% Si aucun joueur n'a perdu, le cout est de 10 fois le nombre de mouvements possibles par sa kalista
evaluer(Board, Joueur, Khan, Cout):-
	possibleMoves(Board, Joueur, Mouvement, Khan),
	mouvementsKalista(Board, Mouvement, Kalista),
	taille(Kalista, T), Cout is T*10,!.

% si aucun joueur n'a perdu et la kalista est immobile, le cout est le nombre de deplacements possibles
evaluer(Board, Joueur, Khan, Cout):-
	possibleMoves(Board, Joueur, Mouvement, Khan),
	listeMouvements(Mouvement, Liste),
	taille(Liste, Cout).

% Si aucun mouvements n'est possible à cause du khan, on réevalue pour aucun khan
evaluer(Board, Joueur, _, Cout):- evaluer(Board, Joueur, n, Cout),!.
...
% Fonction max de l'algorithme minMax : s'arrête si la profondeur est null ou negative
...
% Fonction max de l'algorithme minMax : Si aucun mouvement n'est possible, on execute la boucle de la fonction max sont khan
...
% Fonction min de l'algorithme minMax : s'arrête si la profondeur est null ou negative
...
% Fonction min de l'algorithme minMax : Si aucun mouvement n'est possible, on execute la boucle de la fonction max sont khan
...
% La liste des coups d'un joueur, avec pour chacune, son depart, son arrivée et son evaluation
...
% A partir de la liste des coups évalués, récupère le coup avec la plus grande évaluation
meilleurCoup([T], T):-!.
